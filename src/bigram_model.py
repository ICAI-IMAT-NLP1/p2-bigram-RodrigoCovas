...
def bigrams_count_to_probabilities(
    ...
    bigram_probabilities: torch.Tensor = torch.empty(bigram_counts.shape)
    for row_index in range(bigram_probabilities.shape[0]):
        sum_row: torch.Tensor = bigram_counts[row_index].sum()
        bigram_probabilities[row_index] = bigram_counts[row_index] / sum_row
...
def calculate_neg_mean_log_likelihood(
    ...
    # TODO
    mean_log_likelihood: float = -total_log_likelihood/n_words
...
def generate_name(
    ...
    generated_name: str = ""
    current_char_idx = char_to_idx[start_token]

    # Iterate to build the name
    # TODO
    while len(generated_name) < max_length:
        current_char_idx = char_to_idx[current_char]
        current_char = sample_next_character(current_char_idx, bigram_probabilities, idx_to_char)
        if current_char == end_token:
            break
        if current_char != start_token and current_char != end_token:
...
def calculate_log_likelihood(
    ...
    """
    # Add start and end characters to the word Initialize log likelihood
    log_likelihood_f = 0
    ...
    for i in range(len(word) + 1):
        if i < len(word):
            next_char = word[i].lower()
        else:
            next_char = end_token
        idx_0: int = char_to_index[prev_char]
        idx_1: int = char_to_index[next_char]
        log_likelihood_f += torch.log(bigram_probabilities[idx_0, idx_1] + 1e-8)
        prev_char = next_char


    return log_likelihood_f
